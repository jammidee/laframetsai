...
const initJSONVars = async () => {
  try {
    ...
    await fs.access(path.join(__dirname, '../../app/helpers/initvars.json'), fs.constants.F_OK);
    ...
    await fs.writeFile(path.join(__dirname, "../../app/helpers/initvars.json"), JSON.stringify(data));
  }
};

export default initJSONVars;